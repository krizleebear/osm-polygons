...
# Convert OSM Landuse stream to GeoParquet using DuckDB
# Usage: ./scripts/convert_landuse_to_parquet.sh <COUNTRY_CODE> <INPUT_STREAM> <OUTPUT_PARQUET>
...
SQL_TEMPLATE="${SCRIPT_DIR}/export_landuse.sql"
...
echo "============================================================"
echo " Converting Landuse to GeoParquet"
...
TMP_SQL="$(mktemp /tmp/export_landuse_XXXXXX.sql)"
...
echo "============================================================"
echo " OSM Landuse GeoParquet Summary: $OUTPUT_PARQUET"
duckdb -c "
SELECT
    COUNT(*) AS total_landuse,
    COUNT(DISTINCT landuse) AS distinct_landuse_types,
